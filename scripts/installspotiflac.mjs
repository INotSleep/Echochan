...
const DOWNLOAD_BASE = 'https://github.com/ShuShuzinhuu/SpotiFLAC-Module-Version/releases/latest/download';
...
function resolveAssetName(target) {
    if (target === 'windows') return 'SpotiFLAC-Windows.exe';
    if (target === 'linux-x64') return 'SpotiFLAC-Linux-x86_64';
    if (target === 'linux-arm64') return 'SpotiFLAC-Linux-arm64';
    if (target === 'macos') return 'SpotiFLAC-macOS';
...
async function readPackageConfig() {
    ...
    return packageJson.spotiflac ?? {};
}

function resolveInstallDir(config) {
    const rawDir = process.env.SPOTIFLAC_INSTALL_DIR || config.installDir || '.spotiflac';
...
function getInstalledPath(dir) {
    const executablePath = path.join(dir, getExeName('spotiflac'));
...
export async function ensureSpotiFlac(options = {}) {
    ...
    const tempRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'installspotiflac-'));
    ...
    try {
        ...
        const binaryPath = path.join(stagingDir, getExeName('spotiflac'));
        await fs.promises.copyFile(downloadedPath, binaryPath);
        await writeExecutable(binaryPath);

        await fs.promises.writeFile(path.join(stagingDir, 'installspotiflac.json'), JSON.stringify({
            target,
            assetName,
            sourceUrl,
        ...
        return { binaryPath: path.join(finalDir, getExeName('spotiflac')) };
    } catch (error) {
        await removeSafe(stagingDir);
        throw error;
    } finally {
...
async function main() {
    const result = await ensureSpotiFlac({
        force: process.argv.includes('--force')
    });

    console.log(`spotiflac: ${result.binaryPath}`);
}
